...
void SyncPrefs::RegisterProfilePrefs(PrefRegistrySimple* registry) {
  ...
  // MoltBrowser: default sync to managed/disabled. kSyncManaged is the pref
  // backing the SyncDisabled policy; defaulting it to true disables Chrome Sync
  // entirely, matching the removal of Google sign-in.
  registry->RegisterBooleanPref(prefs::internal::kSyncManaged, true);
  registry->RegisterIntegerPref(
