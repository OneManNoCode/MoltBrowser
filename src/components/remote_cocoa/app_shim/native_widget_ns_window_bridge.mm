...
void NativeWidgetNSWindowBridge::CreateWindow(
  ...
  window_class_ = params->window_class;  // MoltBrowser: for vibrancy scoping.
...
void NativeWidgetNSWindowBridge::CreateContentView(
  ...
  // MoltBrowser: real toolbar glass. For the top-level browser window when it
  // is translucent (the kGlassToolbar path makes it so), add a dark
  // NSVisualEffectView vibrancy layer BELOW the compositor. It shows a real
  // frosted-desktop blur wherever the toolbar / tab-strip chrome is transparent;
  // the opaque web-content quad covers the rest, so the page is unaffected.
  // Scoped to kBrowser so bubbles/menus/popups (also translucent) are untouched.
  if (is_translucent_window_ &&
      window_class_ == mojom::WindowClass::kBrowser) {
    NSVisualEffectView* vibrancy =
        [[NSVisualEffectView alloc] initWithFrame:[bridged_view_ bounds]];
    vibrancy.material = NSVisualEffectMaterialHeaderView;
    ...
    [bridged_view_ addSubview:vibrancy
                   positioned:NSWindowBelow
                   relativeTo:compositor_view];
  }
