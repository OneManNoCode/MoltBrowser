...
class REMOTE_COCOA_APP_SHIM_EXPORT NativeWidgetNSWindowBridge
 ...
 private:
  ...
  // MoltBrowser: window class captured at creation, so the vibrancy header
  // (NSVisualEffectView) is only added to the top-level browser window
  // (kBrowser), not to bubbles/menus/popups that are also translucent.
  mojom::WindowClass window_class_ = mojom::WindowClass::kDefault;
