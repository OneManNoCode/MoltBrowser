...
void PrimaryAccountManager::RegisterProfilePrefs(PrefRegistrySimple* registry) {
  ...
  // MoltBrowser: default browser sign-in to disallowed. MoltBrowser is a
  // privacy browser with no Google account integration; defaulting
  // kSigninAllowed to false removes the sign-in button and the associated
  // Google-account plumbing from the UI without ripping out people_page.
