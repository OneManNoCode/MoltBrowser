...
export class PowerBookmarksListElement extends PolymerElement implements
  ...
  private onImportClicked_(event: MouseEvent) {
    event.preventDefault();
    event.stopPropagation();
    // Open MoltBrowser's own Import & Migration settings, deep-linked to the
    // browser import section. The `tabs` extension API is available to this
    // side-panel WebUI (see _api_features.json), so open it in a new tab.
    chrome.tabs.create({url: 'molt://ai-settings/?section=import'});
  }
