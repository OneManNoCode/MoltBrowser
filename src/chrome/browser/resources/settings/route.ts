...
export function getTopLevelRoute(): Route {
  // MoltBrowser: land on Privacy, not People. The People/sync section is
  // hidden (Google sign-in/sync removed), so landing there left the content
  // pane blank and broke view-switching. Privacy is always visible and a
  // fitting landing for a privacy browser; the "Import from another browser"
  // CTA card in settings_main renders above it on this landing route.
