...
function createPageVisibility(): PageVisibility|undefined {
  if (!loadTimeData.getBoolean('isGuest')) {
    // MoltBrowser: In normal (non-guest) mode Chromium returns undefined here,
    // which leaves every settings section visible — including the Google
    // sign-in/sync "people" section. We instead return an explicit visibility
    // map that hides ONLY the people section (Google account/sync) while
    // keeping every other section visible. A partial object would flip all
    // unspecified sections to hidden, so every PageVisibility field declared
    // in the PageVisibility interface is enumerated explicitly below.
    return {
      a11y: true,
      ai: true,
      appearance: true,
