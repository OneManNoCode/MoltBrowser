...
// MoltBrowser: element deps for the "Import from another browser" CTA card
// rendered at the top of the settings landing page (settings_main.html).
...
export class SettingsMainElement extends SettingsMainElementBase {
  ...
  // MoltBrowser: show the "Import from another browser" CTA card only on the
  // top-level settings landing page (chrome://settings/, whose effective
  // route is getTopLevelRoute()). Hidden during search and on any subpage so
  // it appears exactly once, as the first thing shown when the user opens
  // settings, in both the wide and narrow (side-panel) layouts.
  private showImportCta_(): boolean {
    return !this.inSearchMode_ && !!this.lastRoute_ &&
        this.lastRoute_ === this.topLevelEquivalentRoute_;
  }
