...
void ShowImportDialog(Browser* browser) {
  ...
  // MoltBrowser: repoint the stock "Import Bookmarks and Settings…" command
  // (IDC_IMPORT_SETTINGS, surfaced in the 3-dot Bookmarks submenu and the
  // macOS menu-bar Bookmarks menu) to MoltBrowser's detected-browser import
  // flow instead of Chromium's stock importer sub-page. Open the AI settings
  // page deep-linked to the Import & Migration section. We navigate to the
  // internal chrome://molt-ai-settings/ host; the molt:// reverse rewriter
  // displays it as molt://ai-settings/ in the omnibox, and the ?section=import
  // query drives the page's deep-link handler to scroll/focus #importSection.
  GURL import_url(std::string(chrome::kChromeUIMoltAISettingsURL) +
                  "?section=import");
  ShowSingletonTabIgnorePathOverwriteNTP(browser, import_url);
}
