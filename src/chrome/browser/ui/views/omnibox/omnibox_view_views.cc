...
void OmniboxViewViews::InstallPlaceholderText() {
  if (!controller()->edit_model()->keyword_placeholder().empty()) {
    ...
    // MoltBrowser: Always show "Chat with MoltSearch or type a URL" — never
    // mention Google or any other search engine. We never want users to see
    // "Search Google" branding in any context.
    SetPlaceholderText(u"Chat with MoltSearch or type a URL");
  }
